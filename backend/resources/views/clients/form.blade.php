...
<form method="POST" action="{{ $client->exists ? route('web.clients.update', $client) : route('web.clients.store') }}" class="card form">
    ...
    <div class="fields">
        ...
        <div class="field"><label>RNC / ID Fiscal</label><input name="tax_id" value="{{ old('tax_id', $client->tax_id) }}"></div>
        <div class="field"><label>Correo</label><input name="email" type="email" value="{{ old('email', $client->email) }}"></div>
