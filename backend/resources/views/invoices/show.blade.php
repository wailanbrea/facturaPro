...
@php($paymentAmountValue = number_format((float) old('amount', $invoice->balance_due), 2, '.', ''))
            ...
            <div class="actions">
                ...
                @if($invoice->invoice_number)
                    <form method="POST" action="{{ route('web.invoices.generate-pdf', $invoice) }}">
                        @csrf
                        <button class="btn" type="submit">Regenerar PDF</button>
                    </form>
                @endif
            </div>
        @else
        ...
        <form method="POST" action="{{ route('web.invoices.register-payment', $invoice) }}" class="form">
            ...
            <div class="fields">
                <div class="field">
                    ...
                    <input id="pay-amount" name="amount" type="number" step="0.01" min="0.01" max="{{ number_format((float) $invoice->balance_due, 2, '.', '') }}" value="{{ $paymentAmountValue }}" required>
                </div>
                <div class="field">
                    <label>Fecha</label>
            ...
            <table class="table">
                ...
                @foreach($invoice->payments->sortByDesc('payment_date') as $payment)
                    <tr>
                        ...
                        <td>{{ strtolower((string) $payment->method) === 'manual' ? 'Efectivo' : ($payment->method ?: 'Efectivo') }}</td>
